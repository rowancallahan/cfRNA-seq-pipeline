        ...
        for variant in myvcf:
            if variant[0] != "#":
                ...
                ### Create FORMAT and SAMPLE strings with DP included.
                    ...
                    ### I'm cheating here and just adding .01 for filtering tumor only
                    ### assays.  This really should be added as an argument to the 
                    ### Galaxy wrapper.
                    if args.filtering:
                        raise Exception("This option is not valid for tumor-only VCF's.")
                    elif float(tumor_af) >= .01:
                        handle_out.write('\t'.join(['\t'.join(split_variant[:8]), format, tumor, '\n']))
