...
BIN_DIR="/mnt/lustre1/CompBio/bin"
### Where the genome FASTA's are.
DB_DIR="/mnt/lustre1/CompBio/genomic_resources/genomes/hg19/gsnap"
### The reference genome.
REF_FA="$DB_DIR/Homo_sapiens_assembly19.fasta"
### Location of PICARD
...
GENOME="hg19"
### Gene set.
GTF="/mnt/lustre1/CompBio/genomic_resources/gtf/Homo_sapiens.GRCh37.75.gtf"
### Directory containing project structure.
PROJECT_DIR="/mnt/lustre1/users/letaw/projs/beataml"
PROJ_NAME="beataml"
...
if [ $1 == 66 ]
then
    rm -r $PROJECT_DIR/fastqc $PROJECT_DIR/gsnap $PROJECT_DIR/rg $PROJECT_DIR/dups $PROJECT_DIR/htseq
    exit 1
fi

### Create project directory structure.

mkdir -p $PROJECT_DIR/fastqc $PROJECT_DIR/gsnap $PROJECT_DIR/rg $PROJECT_DIR/dups $PROJECT_DIR/htseq $PROJECT_DIR/logs
...
### Concatenate FASTQ files in to one R1 and one R2.

cd $PROJECT_DIR/raw

if [ ! -e $1\_R1.fastq.gz ]
then
    cat $1/*R1* >> $1\_R1.fastq.gz
fi

if [ ! -e $1\_R2.fastq.gz ]
then
    cat $1/*R2* >> $1\_R2.fastq.gz
fi
	
### Run FastQC.
###
if [ ! -e $PROJECT_DIR/fastqc/$1\_R1_fastqc.html ]
then
    $BIN_DIR/fastqc --noextract -o $PROJECT_DIR/fastqc -t $THREADS $PROJECT_DIR/raw/$1\_R1.fastq.gz $PROJECT_DIR/raw/$1\_R2.fastq.gz
fi

### Run GSNAP
if [ ! -d $PROJECT_DIR/gsnap/$1 ]
then
    mkdir $PROJECT_DIR/gsnap/$1
    $BIN_DIR/gsnap -D $DB_DIR -d $GENOME -s $GENOME --use-sarray=1 -B 5 -m .05 --gunzip -t $THREADS -N 1 -A sam --failed-input $PROJECT_DIR/gsnap/$1/$1.failed --split-output=$PROJECT_DIR/gsnap/$1/$1 $PROJECT_DIR/raw/$1\_R1.fastq.gz $PROJECT_DIR/raw/$1\_R2.fastq.gz 
fi


### Add Read group.

RGID=$(/bin/gzip -cd $PROJECT_DIR/raw/$1_R1.fastq.gz | head -1 | cut -d ':' -f 1-4)
RGPU=$(/bin/gzip -cd $PROJECT_DIR/raw/$1_R1.fastq.gz | head -1 | cut -d ':' -f 10)
RGLB=$(/bin/gzip -cd $PROJECT_DIR/raw/$1_R1.fastq.gz | head -1 | cut -d ':' -f 3-4)
#RGDT=$(echo $1 | cut -d '_' -f 2)
RGSM=$(echo $1 | cut -d '_' -f 3) 

java -jar $PICARD/AddOrReplaceReadGroups.jar INPUT=$PROJECT_DIR/gsnap/$1/$1.concordant_uniq OUTPUT=$PROJECT_DIR/rg/$1\_rg.sam SORT_ORDER=coordinate \
    RGID=$RGID RGLB=$RGLB RGPL="illumina" RGPU=$RGPU RGSM=$RGSM RGDS="beataml" MAX_RECORDS_IN_RAM=5000000


### Mark Duplicates

java -jar $PICARD/MarkDuplicates.jar INPUT=$PROJECT_DIR/rg/$1\_rg.sam OUTPUT=$PROJECT_DIR/dups/$1\_rg_dups.sam M=$PROJECT_DIR/dups/$1\_rg_dups.metrics CREATE_INDEX=TRUE MAX_FILE_HANDLES=25000 MAX_RECORDS_IN_RAM=5000000


### Count gene set/alignment overlap.

htseq-count -s $STRANDED -i gene_name $PROJECT_DIR/dups/$1\_rg_dups.sam $GTF > $PROJECT_DIR/htseq/$1.counts
