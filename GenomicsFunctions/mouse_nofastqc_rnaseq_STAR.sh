...
# Read groups are contained within a CSV placed within the script directory.  These can be produced in Excel if you want and exported as a CSV.  Please retain ordering of read groups as stated on the MarkDuplicates  help page.
...
####!!!>>>CHECK THIS<<<!!!####
genome="mm10"
REF_FA="$ROOT_DIR/genomic_resources/genomes/$genome/Mus_musculus.GRCm38.dna.primary_assembly.fa.gz"
GTF="/home/exacloud/lustre1/CompBio/genomic_resources/gtf/Mus_musculus.GRCm38.84.gtf"
# file parameters  #
...
#java -jar -Xms76G -Xmx76G $PICARD/AddOrReplaceReadGroups.jar INPUT=$PROJECT_DIR/$out/$SAMP_NAME/$SAMP_NAME\_Aligned.out.sam OUTPUT=$PROJECT_DIR/$out\_rg/$SAMP_NAME\_rg.sam SORT_ORDER=coordinate RGID="$RGID" RGLB="$RGLB" RGPL=$RGPL RGPU="$RGPU" RGSM="$RGSM" RGDS=$RGDS MAX_RECORDS_IN_RAM=10000000
