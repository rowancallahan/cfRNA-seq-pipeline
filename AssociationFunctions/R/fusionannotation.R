...
annoCivic_fusions <- function(fusion_bedpe_dt, resource_vardir){
  ...
  #civic=fread("/Users/patterja/Workspace/FusionAnnotation/01-Mar-2017-ClinicalEvidenceSummaries.tsv")
  ...
  #get unique combo-gene names only without "Type 1" or other stuff
  ...
  lsUniq_func <-{function(x) list(unique(x))} 
  fusion_civic_byvar_dt=fusion_civic_dt[ , lapply(.SD, lsUniq_func), by = list(variant)]
  
  #some start and stop overlapping duplicates
  fusion_civic_byvar_dt$start=lapply(fusion_civic_byvar_dt$start, min)
  fusion_civic_byvar_dt$start2=lapply(fusion_civic_byvar_dt$start2, min)
  fusion_civic_byvar_dt$stop=lapply(fusion_civic_byvar_dt$stop, max)
  fusion_civic_byvar_dt$stop2=lapply(fusion_civic_byvar_dt$stop2, max)
  
  
  #GenomicRanges for overlap of civic
  fusion_civicleft_gr=GRanges(seqnames = unlist(fusion_civic_byvar_dt$chromosome), ranges = IRanges(start=unlist(fusion_civic_byvar_dt$start), end = unlist(fusion_civic_byvar_dt$stop)))
  fusion_civicright_gr=GRanges(seqnames = unlist(fusion_civic_byvar_dt$chromosome2), ranges = IRanges(start=unlist(fusion_civic_byvar_dt$start2), end = unlist(fusion_civic_byvar_dt$stop2)))
  

  #GenomicRanges fusion bedpe
  fusions_left_gr=GRanges(seqnames = fusion_bedpe_dt$chrom1, ranges = IRanges(start=fusion_bedpe_dt$start1, end = fusion_bedpe_dt$end1))
  fusions_right_gr=GRanges(seqnames = fusion_bedpe_dt$chrom2, ranges = IRanges(start=fusion_bedpe_dt$start2, end = fusion_bedpe_dt$end2))
  
  #OVERLAPS
  leftleft_hitidx=suppressWarnings(findOverlaps(fusions_left_gr, fusion_civicleft_gr))
  leftright_hitidx=suppressWarnings(findOverlaps(fusions_left_gr, fusion_civicright_gr))
  rightright_hitidx=suppressWarnings(findOverlaps(fusions_right_gr, fusion_civicright_gr))
  rightleft_hitidx=suppressWarnings(findOverlaps(fusions_right_gr, fusion_civicleft_gr))
  

  # create empty data.table
  civicannotated_dt=data.table(matrix(nrow=nrow(fusion_bedpe_dt), ncol=ncol(fusion_civic_byvar_dt), data="NA")) #the data=NA prevents warnings later
  trueanno_ls=leftleft_hitidx[leftleft_hitidx %in% rightright_hitidx]
  invertanno_ls=leftleft_hitidx[leftright_hitidx %in% rightleft_hitidx]
  colnames(civicannotated_dt)=paste("CIVIC", colnames(fusion_civic_byvar_dt), sep = "_")
  if (length(trueanno_ls)>0) {
    civicannotated_dt[queryHits(trueanno_ls)]=lapply(fusion_civic_byvar_dt[(subjectHits(trueanno_ls))], as.character)
    }
  if (length(invertanno_ls) >0) {
    civicannotated_dt[queryHits(trueanno_ls)]=lapply(fusion_civic_byvar_dt[(subjectHits(trueanno_ls))], as.character)
    }
  
 
  fusion_bedpe_annoCivic=cbind(fusion_bedpe_dt, civicannotated_dt)
